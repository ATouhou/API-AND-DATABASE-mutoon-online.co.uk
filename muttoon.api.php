...
class MysqliDb
{
    ...
    public function orderBy($orderByField, $orderbyDirection = "DESC", $customFields = null)
    {
        ...
        // We are adding prefix only if table is enclosed into `` to distinguish aliases
        // from table names
...
if(isset($_REQUEST['authors'])){
    $datafeed = $db->get('authors'); //contains an Array of all users
    //var_dump($datafeed);
    echo json_encode($datafeed);
}
elseif(isset($_REQUEST['books'])){
    $db->where ("status", 1);
    $cols = Array ("bookid", "name_english", "name_arabic", "number_of_pages", "version");
    $datafeed = $db->get('books', null, $cols); //contains an Array of all users
    //var_dump($datafeed);
    echo json_encode($datafeed);
}
elseif(isset($_REQUEST['reciters'])){
    $datafeed = $db->get('reciters'); //contains an Array of all users
    //var_dump($datafeed);
    echo json_encode($datafeed);
}
elseif(isset($_REQUEST['shurooh'])){
    $datafeed = $db->get('shurooh'); //contains an Array of all users
    //var_dump($datafeed);
    echo json_encode($datafeed);
}elseif(isset($_REQUEST['book'])){


    $db->orderBy("page_number","asc");
    $db->where ('book_id', $_REQUEST['book']);
    $datafeed = $db->get('pages'); //contains an Array of all users

    //$datafeed = $db->JsonBuilder()->get('pages'); //contains an Array of all users
    //echo $datafeed;
    echo json_encode($datafeed);


}else{
    echo "Nothing valid requested";
}
